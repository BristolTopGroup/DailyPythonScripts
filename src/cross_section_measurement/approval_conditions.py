...
#     variables = ['ST']
    for variable in variables:
        ...
        svd_data = read_data_from_JSON(svd)['TTJet_unfolded']
        ...
        properties.name = 'compare_unfolding_methods_{0}'.format(variable)
    ...
    for variable in variables:
        ...
        for channel in ['electron', 'muon']:
            ...
            properties.name = 'compare_qcd_control_regions_to_mc_{0}_{1}_channel'.format(
                variable, channel)
            properties.title = 'Comparison of QCD control regions ({0} channel)'.format(channel)
            properties.path = 'plots'
