...
import importlib
from dps.utils.hist_utilities import rebin_2d, fix_overflow
...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        ...
        while current_bin_end < n_bins:
            ...
            self.bin_edges.append(first_hist.GetXaxis().GetBinLowEdge(
                current_bin_end) + first_hist.GetXaxis().GetBinWidth(current_bin_end))
        ...
        self.stabilities_GetBinContent = calculate_stabilities(
            self.h1_rebinned)
        self.n_events_GetBinContent = [
            int(self.h1_rebinned.GetBinContent(i, i)) for i in range(1, len(self.bin_edges))]
    ...
    def test_number_of_bins_equivalence(self):
        self.assertEqual(
            len(self.n_events_GetBinContent), len(self.n_events_Integral))
        self.assertEqual(
            len(self.purities_GetBinContent), len(self.purities_Integral))
        self.assertEqual(
            len(self.stabilities_GetBinContent), len(self.stabilities_Integral))
        pass

    @unittest.skip('reported: https://github.com/BristolTopGroup/DailyPythonScripts/issues/333')
    ...
    def test_purities_equivalence(self):
        for i, p in enumerate(self.purities_GetBinContent):
            self.assertEqual(p, self.purities_Integral[i], msg='Calculated with Integral method purity ' + str(
                self.purities_Integral[i]) + ' is not equal to GetBinContent one ' + str(p) + ' in bin ' + str(i + 1))
        pass

    def test_stabilities_equivalence(self):
        for i, s in enumerate(self.stabilities_GetBinContent):
            self.assertEqual(s, self.stabilities_Integral[i], msg='Calculated with Integral method stability ' + str(
                self.stabilities_Integral[i]) + ' is not equal to GetBinContent one ' + str(s) + ' in bin ' + str(i + 1))
        pass
