...
from dps.config.variable_binning import bin_edges_vis as bin_edges
...
class Test(unittest.TestCase):
    ...
    @unittest.skip('skipping until unfolding_merged_asymmetric.root is sorted out')
    def test_invalid_zero_data(self):
        variable = 'MET'
        channel = 'electron'
        pseudo_data = value_error_tuplelist_to_hist(
            [(0, 0)] * (len(bin_edges[variable]) - 1), bin_edges[variable])
        self.assertRaises(ValueError,  self.dict[channel][variable][
                          'unfolding_object'].unfold, (pseudo_data))
...
#                     print value, '+-', error, '   true:', true_value
    ...
    def test_k_to_tau(self):
        ...
        tau_unfolding_object = self.dict['electron'][
            'MET']['tau_unfolding_object']
