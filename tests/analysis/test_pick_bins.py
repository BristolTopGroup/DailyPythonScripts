...
class Test(unittest.TestCase):
    ...
    def test_n_events(self):
        n_events = [self.h1_rebinned.GetBinContent(
            i, i) for i in range(1, len(self.bin_edges_1))]
        for N in n_events:
            self.assertGreaterEqual(N, self.n_min)

    def test_n_events_reduced_N(self):
        n_events = [self.h2_rebinned.GetBinContent(
            i, i) for i in range(1, len(self.bin_edges_2))]
        for N in n_events:
            self.assertGreaterEqual(N, self.n_min)

    def test_n_events_reduced_spread(self):
        n_events = [self.h3_rebinned.GetBinContent(
            i, i) for i in range(1, len(self.bin_edges_3))]
        for N in n_events:
            self.assertGreaterEqual(N, self.n_min)

    def test_n_events_combined_1_2(self):
        n_events = [self.h1_2_rebinned.GetBinContent(
            i, i) for i in range(1, len(self.bin_edges_1_2))]
        for N in n_events:
            self.assertGreaterEqual(N, self.n_min)

    def test_n_events_combined_1_3(self):
        n_events = [self.h1_3_rebinned.GetBinContent(
            i, i) for i in range(1, len(self.bin_edges_1_3))]
        for N in n_events:
            self.assertGreaterEqual(N, self.n_min)
