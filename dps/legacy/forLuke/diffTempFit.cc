...
double lumi = 4973.4;
  ...
  //scale for bin
  double bin2bin[5] =  {1.1319 , 1.0636 , 0.934024 , 0.902389 , 0.98126};
...
//  TH1D* signal_sys = (TH1D*) f_->Get("metEta_h_"+jet_num+"signal");  
//  TH1D* wjets_sys = (TH1D*) f_sys->Get("metEta_h_"+jet_num+"wjets");
//  TH1D* zjets_sys = (TH1D*) f_sys->Get("metEta_h_"+jet_num+"zjets");
...
double Ndata = DATA->Integral();
  ...
  getSignalFromHis(f_central, jet_num, syst, nbins, ttbar_Vec, ttbar_err_Vec);// Get ttbar, single-top histrograms form MC with entral selection 
...
theor->SetBinContent(met+1, (bin2bin[met]*theorNum/157.5/width)); 

sysup->SetBinContent(met+1, (bin2bin[met]*upNum/157.5/width)); 
sysdown->SetBinContent(met+1, (bin2bin[met]*downNum/157.5/width)); 
mup->SetBinContent(met+1, (bin2bin[met]*mupNum/157.5/width)); 
mdown->SetBinContent(met+1, (bin2bin[met]*mdownNum/157.5/width));
...
double tempStat[5] = {0,0,0,0,0};

double JESup[5] = {0.00018686 , 0.000254873 , 0.000181984 , -0.000525503 , 3.26796e-05}; 
double JESdown[5] = {0.000223406 , -0.000362 , 0.000248638 , -0.000246032 , 6.08773e-05};

double METup[5] = {0.000385347 , 3.01644e-05 , -0.000476814 , 2.71308e-05 , 2.51081e-05};
double METdown[5] = {0.000366172 , -0.000133105 , -0.000195811 , -3.46826e-05 , -3.79334e-06};

double PUup[5] = {0.000132928 , -2.48779e-05 , 6.61007e-07 , -1.04251e-06 , -5.35471e-05};
double PUdown[5] = {-6.59201e-05 , -6.32625e-06 , 1.66483e-05 , 1.09521e-05 , 1.92887e-05};

double Bup[5] = {-3.1983e-06 , 3.08789e-05 , -7.06707e-06 , -3.20028e-06 , -5.35066e-06};
double Bdown[5] = {-9.95133e-06 , 4.11996e-05 , -1.62558e-05 , -7.1995e-06 , 7.56417e-07};

double Qallup[5] = {-0.000130589 , -2.90699e-05 , 0.000226989 , -3.44631e-05 , -1.84942e-05};
double Qalldown[5] = {0.000957462 , -0.000710205 , 0.000534231 , -0.000654179 , -5.00956e-05};

double Mallup[5] = {0.000307849 , -0.000103324 , -0.000125457 , -5.91573e-05 , -8.203e-06};
double Malldown[5] = {0.00023947 , -0.000892247 , 0.000806758 , -5.76717e-05 , -0.000126811};

double singletD[5] = {7.87955e-06 , -4.22954e-06 , -1.23691e-05 , -4.43155e-06 , 6.75271e-06};
double singletU[5] = {-8.08991e-06 , 4.34e-06 , 1.26758e-05 , 4.5571e-06 , -6.9254e-06};

double idUp[5] = {5.99454e-05 , 0.000150228 , 0.000116607 , -0.000280843 , 2.13493e-05};
double idDown[5] = {7.70397e-05 , 0.000146588 , 9.50871e-05 , -0.000283887 , 2.71862e-05};


double lumiD[5] = {5.67361e-05 , 0.00018803 , 0.000108069 , -0.000301307 , 2.43164e-05};
double lumiU[5] = {6.65391e-05 , 0.000143872 , 0.000125036 , -0.000292431 , 2.34648e-05};

double pdf[5] = {2.71e-05 , 3.66e-05 , 1.28e-05 , 1.33e-05 , 1.03e-05};

double stat[5] = {0.000236782 , 0.0003032 , 0.000233202 , 6.27219e-05 , 8.94673e-05};

double up[5], down[5];

for(int bin = 0; bin < 5; bin++ ){
down[bin] =sqrt(pow(JESdown[bin],2)+pow(Qalldown[bin],2)+pow(Malldown[bin],2)+pow(singletD[bin],2)+pow(lumiD[bin],2)+pow(pdf[bin],2)+pow(METdown[bin],2)+pow(stat[bin],2)+pow(tempStat[bin],2)+pow(PUdown[bin],2)+pow(Bdown[bin],2)+pow(idDown[bin],2));
up[bin] =sqrt(pow(JESup[bin],2)+pow(Qallup[bin],2)+pow(Mallup[bin],2)+pow(singletD[bin],2)+pow(lumiD[bin],2)+pow(pdf[bin],2)+pow(METup[bin],2)+pow(stat[bin],2)+pow(tempStat[bin],2)+pow(PUup[bin],2)+pow(Bup[bin],2)+pow(idUp[bin],2));


y[bin] = (bin2bin[bin]*y[bin])/totXsect;
...
//central fit
 double central[5] = {0.00616768 , 0.0143576 , 0.0116077 , 0.00554829 , 0.00216365};
...
nominal->SetBinContent(i,central[i-1]);
  ...
  //plot colors
     ...
     sysup->SetLineColor(kBlue);
     sysdown->SetLineColor(kRed+1);
     mup->SetLineColor(kMagenta+3);
     mdown->SetLineColor(kGreen + 4);
     //theor->SetFillColor(kRed+1);
     nominal->SetLineColor(kWhite);
     
     theor->SetLineStyle(7);
...
for(int bin = 1; bin<=5; bin++){
  ...
  // std::cout << measured->GetBinError(bin) << " , ";
}
std::cout << " " << std::endl;
cout.precision(3);
for(int bin = 1; bin<=5; bin++){
std::cout << 100*measured->GetBinContent(bin) << " \\pm "  << 100*stat[bin-1] << " (fit)^{+" << 100*up[bin-1] << "}_{-"<<100*down[bin-1] << "} (sys)" << std::endl;

//std::cout << 100*(measured->GetBinContent(bin)-nominal->GetBinContent(bin))/measured->GetBinContent(bin)  << "\\\% & " ;
   //std::cout << 100*(up[bin-1])/measured->GetBinContent(bin)  << "\\\% & " ;
   //std::cout << 100*(measured->GetBinError(bin)/measured->GetBinContent(bin)) << "\\% & ";
}
...
//   tleg->AddEntry(mdown    , "t#bar{t} (matching threshold 10 GeV)"  , "l");
//   tleg->AddEntry(mup    , "t#bar{t} (matching threshold 40 GeV)"  , "l"); 
...
sysup->GetYaxis()->SetTitle("#frac{1}{#sigma} #frac{#partial #sigma}{#partial MET} [GeV^{-1}]");sysup->GetYaxis()->SetTitleSize(0.05);
sysup->GetXaxis()->SetTitle("MET [GeV]"); sysup->GetXaxis()->SetTitleSize(0.05);
...
c->SaveAs("../pasPlots/compGen.png");
c->SaveAs("../pasPlots/compGen.pdf");
...
void getSignalFromHis(TFile* inputFile, TString& jet_num, TString& syst,int& nxbins, vector<double>& vect, vector<double>& vect_err){
  ...
  TH1F* his_mc_array = (TH1F*) inputFile->Get("metEta_h_"+jet_num+syst);  
